...
assert remove_duplicates_set("Yoona") == "Yona"
assert remove_duplicates_set("Capilano") == "Capilno"
assert remove_duplicates_set("Flower") == "Flower"
...
# Your unit tests
assert gem_counting("f", "abcd") == 0
assert gem_counting("abcd", "abcde") == 4
assert gem_counting("abcd", "AbcD") == 2
...
def students_id(ids):
    ...
    students_seen = set()
    for id in ids:
        if id not in students_seen:
            students_seen.add(id)
            students_shuttle += 1
    return students_shuttle
...
def students_id_occurrences(ids):
    """
     This function returns the occurence of each diffrent.
    ...
    Parameters:
    - ids (lst): A list of student id numbers.

    Returns:
    - (dict): The occurences of each different student.
...
# Your unit tests
students_id_occurences(['002', '003', '001', '004', '012']) returns {'002': 1, '003': 1, '001': 1, '004': 1, '012': 1}}
students_id_occurences(['002', '003', '001', '012', '003', '001']) returns {'002': 1, '003': 2, '001': 2, '012': 1}
...
# Your unit tests
assert word_frequency("I am Yoona. I am happy.") == {'I': 2, 'am': 2, 'Yoona.': 1, 'happy.': 1}
